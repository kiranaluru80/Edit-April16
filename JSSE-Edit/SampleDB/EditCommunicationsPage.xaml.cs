...
namespace SampleDB
{
    public partial class EditCommunicationsPage : ContentPage
    {
        ...
        public EditCommunicationsPage(int Org_Id, string jsseid, EditUserListModel edituserList)
        {
            InitializeComponent();


           // await ReadDataFromJson();
            Task.Run(() => this.ReadDataFromJson()).Wait();
        
            DynamicCategaryDesign(arrayobj);

            saveAsPdfOrPrint.Clicked += (sender, e) => {
            ...
            tapGestureRecognizer.Tapped += (s, e) => {
                ...
                Navigation.PushAsync(new JobInformationPage(edituserList));
            };
        ...
        public Task<int> ReadDataFromJson()
        {
            ...
            using (var reader = new System.IO.StreamReader(stream))
            {
                ...
                arrayobj = new List<Category>();
                if (arrayobj.Count() > 0){
                    for (int i = 0; i < arrayobj.Capacity; i++)
                    {
                        arrayobj.Remove(arrayobj[i]);
                    }
                }
